...
export class PaymentService {
  ...
  async requestPaymentLink(telegramUserId: number): Promise<PaymentLinkResult> {
    ...
    const now = new Date()
    const existingPendingPayment = await this.findFreshPendingPayment(
      telegramUserId,
      now,
    )
    ...
    const rateLimitUntil = getPaymentRateLimitUntil(
      user?.lastPaymentLinkCreatedAt,
    )
